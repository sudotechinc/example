...
public class TableEditor extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TableEditor frame = new TableEditor();
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					List<Table> sessions = null;

					if (lastName != null && lastName.trim().length() > 0) {
						sessions = tableDAO.searchSessions(lastName);
					} else {
						sessions = tableDAO.getAllSessions();
					}
					
					// create the model and update the "table"
					TableEditorTableModel model = new TableEditorTableModel(sessions);
					...
					*/
					...
					JOptionPane.showMessageDialog(TableEditor.this, "Error: " + exc, "Error", JOptionPane.ERROR_MESSAGE); 
				}
